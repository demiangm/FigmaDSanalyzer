...
imports_1.jsonDataFiles.forEach((file) => {
...
figma.ui.onmessage = (msg) => __awaiter(void 0, void 0, void 0, function* () {
    switch (msg.type) {
        case 'analyze-selection':
            yield analyzeSelection();
            break;
        case 'update-design-system':
            designSystemConfig = msg.config;
            yield figma.clientStorage.setAsync('designSystemConfig', designSystemConfig);
            break;
        case 'get-design-system':
            const savedConfig = yield figma.clientStorage.getAsync('designSystemConfig');
            if (savedConfig) {
        ...
        case 'generate-config-files':
            yield generateConfigurationFiles();
            break;
        case 'close-plugin':
            figma.closePlugin();
            break;
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const node of selection) {
            if (node.type === 'FRAME' || node.type === 'COMPONENT' || node.type === 'INSTANCE') {
                const report = yield analyzeFrame(node);
                reports.push(report);
            }
        }
        figma.ui.postMessage({
...
function analyzeFrame(frame) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield analyzeNodeRecursively(frame, analysis);
...
function analyzeNodeRecursively(node, analysis) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const nodeAnalysis = (0, analyzer_1.analyzeNode)(node, Object.assign(Object.assign({}, designSystemConfig), { components: designSystemData.components, styles: designSystemData.styles }), skipComponentAnalysis);
        // Aggregate results
        ...
        if ('children' in node && node.children) {
            for (const child of node.children) {
                yield analyzeNodeRecursively(child, analysis);
            }
        }
        ...
        if (node.type === 'INSTANCE') {
            ...
            if ('children' in node && node.children) {
                for (const child of node.children) {
                    yield analyzeNodeRecursively(child, instanceAnalysis);
                }
            }
