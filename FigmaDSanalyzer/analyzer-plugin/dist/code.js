...
const dataLoader_1 = require("./src/dataLoader");
// Show the plugin UI
...
async function loadDesignSystemData() {
    try {
        const data = await (0, dataLoader_1.loadDataFiles)();
        ...
        figma.ui.postMessage({
            type: 'analysis-error',
            message: 'Erro ao carregar dados do Design System: ' + error.message
        });
    }
}
async function analyzeSelection() {
    ...
    for (const node of selection) {
        if (node.type === 'FRAME' || node.type === 'COMPONENT' || node.type === 'INSTANCE') {
            ...
            await createAnalysisCard(report, node);
        }
    }
    figma.ui.postMessage({
...
async function analyzeNodeRecursively(node, analysis) {
    ...
    const nodeAnalysis = (0, analyzer_1.analyzeNode)(node, componentsData, stylesData, skipComponentAnalysis);
    // Aggregate results
    ...
    if (node.type === 'INSTANCE') {
        ...
        if (mainComponent && !componentsData.some(data => Object.values(data.components).some(comp => comp.key === mainComponent.key))) {
...
async function createAnalysisCard(report, frame) {
    const cardWidth = 300;
