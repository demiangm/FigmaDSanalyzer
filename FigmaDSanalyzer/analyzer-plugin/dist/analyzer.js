...
function analyzeNode(node, componentsData, stylesData, skipComponentAnalysis = false) {
    const result = {
...
function analyzeComponents(node, componentsData, result) {
    ...
    if (node.type === 'COMPONENT' || node.type === 'INSTANCE') {
        ...
        if (node.type === 'INSTANCE') {
            ...
            if (mainComponent) {
                const isFromDesignSystem = componentsData.some(data => Object.values(data.components).some(comp => comp.key === mainComponent.key));
        ...
        // Para componentes principais, verifica se estão no DS
            ...
            const isFromDesignSystem = componentsData.some(data => Object.values(data.components).some(comp => comp.key === node.key));
            if (isFromDesignSystem) {
                result.components.compliant++;
            }
            else {
...
function analyzeColors(node, stylesData, result) {
    // Verifica fills
...
function analyzeFonts(node, stylesData, result) {
    if (node.type === 'TEXT') {
        const textNode = node;
        result.fonts.total++;
...
function analyzeEffects(node, stylesData, result) {
